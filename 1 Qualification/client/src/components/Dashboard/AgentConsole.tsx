...
import { Zap, Send, Loader2, ChevronDown } from 'lucide-react';
...
export default function AgentConsole({ onAccountsUpdate }: Props) {
  ...
  return (
    <div className="rounded-xl border border-gray-200 overflow-hidden">
      ...
      <button
        onClick={() => setOpen((o) => !o)}
        className="w-full bg-gray-50 px-4 py-2.5 flex items-center gap-2 hover:bg-gray-100 transition-colors"
      >
        <Zap className="w-4 h-4 text-[hsl(24,95%,53%)] shrink-0" />
        ...
        {streaming && <Loader2 className="w-3.5 h-3.5 text-slate-400 animate-spin ml-auto mr-1" />}
        <ChevronDown
          className={`w-4 h-4 text-slate-400 ml-auto transition-transform duration-200 ${open ? 'rotate-180' : ''}`}
        />
      </button>
      ...
      <AnimatePresence initial={false}>
        {open && (
          ...
          >
            ...
            <div
              ref={terminalRef}
              className="bg-gray-950 min-h-[80px] max-h-48 overflow-y-auto p-3 font-mono text-xs leading-relaxed"
            >
              <AnimatePresence initial={false}>
                {lines.map((line, i) => (
                  <motion.div
            ...
            <div className="bg-gray-950 border-t border-gray-800 px-4 py-3 flex gap-2">
              <input
                ...
                placeholder='e.g. "set churn threshold to 40%" or "change renewal window to 90 days"'
            ...
            <div className="bg-white border-t border-gray-100 px-4 py-2.5 flex gap-2 flex-wrap">
              {[
                'set churn threshold to 40%',
                'change renewal window to 90 days',
                'set expansion threshold to 130%',
              ].map((example) => (
                <button
                  key={example}
